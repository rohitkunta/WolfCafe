    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable())
            .authorizeHttpRequests(authorize -> authorize
                ...
                .requestMatchers(HttpMethod.POST, "/api/orders").permitAll()         // Allow guest order placement
                .requestMatchers("/api/items/**").permitAll()                        // Allow guest to see items
                .requestMatchers("/api/taxRate/**").permitAll()                      // Allow guest to fetch tax rate
                .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()              // Allow preflight requests
                .anyRequest().authenticated()                                        // Require auth for anything else
            )
